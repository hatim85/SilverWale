...
export const paymentVerification = async (req, res) => {
  ...
  if (isAuthentic) {
    try {
      ...
      const decodedProducts = JSON.parse(decodeURIComponent(products)); // [{productId, quantity}]
      ...
      const itemsTotal = orderLines.reduce((sum, l) => sum + l.unitPriceAtPurchase * l.quantity, 0);
